...
export class CountriesSeeder implements Seeder {
    ...
    public arr = [{'name': 'Afghanistan', 'alpha-2': 'AF', 'country-code': '004'}, {
        'name': 'Åland Islands',
        'alpha-2': 'AX',
        'country-code': '248'
    ...
    constructor(
        @InjectModel(Countries.name) private readonly model: Model<Countries>,
    ...
    async seed(): Promise<any> {
        const finalArr = [];
        this.arr.map(async item => {
            const originFilepath = path.resolve(FileService.storagePath, 'flags');
            const originFilename = `${(item['alpha-2']).toLowerCase()}.png`;
            const targetPath = await lastValueFrom(this.fileService.customUploadImg(
                fs.readFileSync(originFilepath + '/' + originFilename),
                FileSystemAdapter.targetDir,
                'fl',
                originFilename
            ));
            const _item = new this.model({
                name: item.name,
                iso_code: item['alpha-2'],
                filepath: targetPath
            })
            finalArr.push(_item);
        })
        return this.model.insertMany(finalArr);
    }
