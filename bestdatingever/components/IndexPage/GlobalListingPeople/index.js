      ...
      >
        ...
        {requestStatus === 'success' && (
          !!list.length ? list.map(person => (
            ...
            <div className={cn(
              styles['no-results'],
              'title-xs text-italic color-grey-600'
            )}>
              There are&nbsp;
              <span className={styles['no-results__underline']}>
                no results for the specified parameters, &nbsp;
              </span>
              <span className={styles['no-results__underline']}>
                please try to change one or more parameters
              </span>.
            </div>
          )
        )}
