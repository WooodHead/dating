...
const refuseSharingFromUser = createAsyncThunk('user/refuseSharingFromUser', async (userId, { rejectWithValue }) => {
    try {
        const { data } = await api.userPrivateAlbums.refuseSharedAlbums(userId);
        toast.success(data.message);
        return data;
    } catch (err) {
        const res = err.response.data;
        if (res?.statusCode === 400) {
            toast.warning(res?.message[0]);
            throw err;
        } else if (res?.statusCode === 403) {
            return rejectWithValue(res);
        } else if (res?.statusCode === 418) {
