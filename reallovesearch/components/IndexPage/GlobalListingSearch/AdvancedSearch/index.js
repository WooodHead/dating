...
function AdvancedSearch({ control }) {
  ...
  const [generalIsVisible, setGeneralIsVisible] = useState(false);
  const [bodyIsVisible, setBodyIsVisible] = useState(false);
  const [nutritionIsVisible, setNutritionIsVisible] = useState(false);
  ...
  return (
    <>
      {isOpen && (
        <>
          <AdvancedSearchTitle onClick={() => setGeneralIsVisible(!generalIsVisible)}>
            General description
            <div className={cn(styles.icon, 'd-flex justify-content-center align-items-center')}>
              <IconChevronRight className={cn('rotate-90', { 'rotate-270': generalIsVisible })} color="#031539" />
            </div>
          </AdvancedSearchTitle>
          <div className={cn(styles.row, { [styles['row--active']]: generalIsVisible })}>
            <div>
          ...
          <AdvancedSearchTitle onClick={() => setBodyIsVisible(!bodyIsVisible)}>
            Body parameters
            <div className={cn(styles.icon, 'd-flex justify-content-center align-items-center')}>
              <IconChevronRight className={cn('rotate-90', { 'rotate-270': bodyIsVisible })} color="#031539" />
            </div>
          </AdvancedSearchTitle>
          <div className={cn(styles.row, { [styles['row--active']]: bodyIsVisible })}>
            <div>
              <div className="text-md text-semibold">
                Hair color
              </div>
          ...
          <AdvancedSearchTitle onClick={() => setNutritionIsVisible(!nutritionIsVisible)}>
            Nutrition, habits
            <div className={cn(styles.icon)}>
              <IconChevronRight className={cn('rotate-90', { 'rotate-270': nutritionIsVisible })} color="#031539" />
            </div>
          </AdvancedSearchTitle>
          <div className={cn(styles.row, { [styles['row--active']]: nutritionIsVisible })}>
            <div>
              <div className="text-md text-semibold">
                Diet
              </div>
      ...
      <div
        className="d-flex align-items-center text-md text-semibold text-gilroy cursor-pointer mb-3"
